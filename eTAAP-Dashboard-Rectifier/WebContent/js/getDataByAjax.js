...
function retrievedCIResults(response)
{
	document.getElementById('ci-chart-container').innerHTML = "";
	var jsonResult = jQuery.parseJSON(response);
	//alert("JSON Result "+jsonResult);
	var ci = jsonResult.data.CI;
	//alert(ci.buildDateCSV);
	ciChart(ci.buildNumberCSV,ci.passCountCSV,ci.failCountCSV,ci.skipCountCSV,ci.buildDateCSV);
}
...
//			var literal = "<li  id='envId-" + value["name"] + "\' class='filter-submenu'  value=\'" + value[systemApi=='Jira' ? "mapId" : "name"] + "' onclick=\"getLeftMenuItems('env',\'"+value["name"]+"\',this);getCiResultsAjaxCall('env',null);\">" + value["name"] + "</li>";
			var literal = "<li  id='envId-" + value["name"] + "\' class='filter-submenu'  value=\'" + value[systemApi=='Jira' ? "mapId" : "envId"] + "' onclick=\"getLeftMenuItems('env',\'"+value["name"]+"\',this);getCiResultsAjaxCall('env',"+value["envId"]+");\">" + value["name"] + "</li>";
...
function getDefectsAjaxCall(which,input){
	...
	$.ajax({url:uri,
			...
			success:function(result)
				{
				retrievedDefectsResults(result);
