...
<html>
<head>
	<title>eTAAP Dashboard - Defects</title>
	...
	<script type="text/javascript" src='<c:url value="/js/ci_chart.js" />'></script>
...
<body onload="init();fillCsvChat('defects');">
	...
	<!-- main content container starts here -->
		...
		<form id="default_form" action="filter" method="post">
	    	...
	    	<section>
				...
				<script type="text/javascript">
					$('div .filters-container').addClass('filters-container no-border');
				</script>
	...
	<script type="text/javascript">
		...
		$('#tcm-chart-container').hide();
		$('#defects').attr('class','active')
		$("#defects").siblings().attr('class','deactive-link')
		...
		$('#displayName').html('Defects :&nbsp;');

		//filters-container no-border
		document.getElementById('defects-chart-container').style.display = 'block';
		document.getElementById('ci-chart-container').style.display = 'none';
		</script>
</body>
</html>
